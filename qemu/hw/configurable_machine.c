		...
		for (elm = (QDictEntry *) qdict_first(reg); elm != NULL; elm = (QDictEntry *) qdict_next(reg, elm)) {
			...
			if (separator == NULL) {
				...
				*separator = '\0';
				...
				else if (strcmp(separator+1, "sys") == 0) {
					mode = ARM_CPU_MODE_SYS;
				}
				...
				if (mode != 0) {
					if (strcmp(regname, "sp") == 0) {
						helper_set_r13_banked(cpu, mode, regvalue);
					}
					else if (strcmp(regname, "lr") == 0) {
						helper_set_r14_banked(cpu, mode, regvalue);
					}
					else if (strcmp(regname, "spsr") == 0) {
						helper_set_spsr_banked(cpu, mode, regvalue);
					}
